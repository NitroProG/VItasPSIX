...
namespace Psico
{
    public partial class teor2 : Form
    {
        ...
        private void FormLoad(object sender, EventArgs e)
        {
            ...
            kolvoCb = Convert.ToInt32(new SQL_Query().GetInfoFromBD("select count(*) from CBFormFill where zadacha_id = " + Program.NomerZadachi + " and FormCB = 'Teor'")) + 1;

            // Определение переменной для равномерных колонок с checkbox
            stolb = kolvoCb / 2;

            // Выбор количества правильных ответов у задачи
            kolvootv = Convert.ToInt32(new SQL_Query().GetInfoFromBD("select count(*) from vernotv where zadacha_id = " + Program.NomerZadachi + " and FormVernOtv = 'Teor'"));

            // Выбор количества данных в таблице БД
            AllProsmotrMerodiks = Convert.ToInt32(new SQL_Query().GetInfoFromBD("select count(*) from OtvSelected where users_id = " + Program.user + " and FormOtvSelected = 'Teor'"));
            ...
            // Заполнение datagr
            new SQL_Query().UpdateDatagr("select CB from CBFormFill where zadacha_id = " + Program.NomerZadachi + " and FormCB = 'Teor'","CBFormFill",datagr);
            ...
            // Заполнение datagr2
            new SQL_Query().UpdateDatagr("select otv from vernotv where zadacha_id = " + Program.NomerZadachi + " and FormVernOtv = 'Teor'","vernotv",datagr2);
            ...
            // Заполнение datagr1
            new SQL_Query().UpdateDatagr("select name_otv from Lastotv where users_id = " + Program.user + " and Form_otv = 'Teor'","Lastotv",datagr1);
            ...
            // Заполнение datagr5
            new SQL_Query().UpdateDatagr("select InfoSelected from OtvSelected where users_id = " + Program.user + " and FormOtvSelected = 'Teor'","OtvSelected",datagr5);
            ...
            // Выбор количества правильных ответов у задачи
            kolvovibor = Convert.ToInt32(new SQL_Query().GetInfoFromBD("select count(*) from Lastotv where users_id = " + Program.user + " and Form_otv = 'Teor'"));
        ...
        private void GetCBChecked()
        {
            ...
            new SQL_Query().DeleteInfoFromBD("delete from Lastotv where users_id = " + Program.user + " and Form_otv = 'Teor'");
